...
func _ready() -> void:
	...
	label = $"../CardManager/OpponentTotal"
	update_score()
...
func add_card_to_hand(card: Node2D, hidden: bool = false) -> void:
	if hidden and not has_hidden_card:
		...
		sprite.texture = load("res://Assets/Design/Icons/Card[LOOP_DE_LOOP].png")
		sprite.visible = true
	else:
...
func reveal_hidden_card() -> void:
	...
	sprite.texture = load("res://Assets/Cards/Cards (1 - 11)/"+ str(hidden_card_value) + ".png")
